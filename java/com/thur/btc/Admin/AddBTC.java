...
public class AddBTC implements CommandExecutor {
    @Override
            ...
            if(p.hasPermission("thbtc.admin")){
                if(args.length == 2){
                    ...
                    if(Global.isOnline(player_name, p)){
                        ...
                        p.sendMessage("§aForam creditados '" + btcs + "BTC(s)' ao jogador '" + p.getDisplayName() + "'");
                    }else{
                        p.sendMessage("§cEste jogador esta offline.");
                    }
                }else{
                    p.sendMessage("§cUtilize: /addbtc <player> [btc]");
                }
            }
        }
        return false;
